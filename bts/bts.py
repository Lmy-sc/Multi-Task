...
class silog_loss(nn.Module):
    ...
    def forward(self, depth_est, depth_gt, mask):
        epsilon = 1e-6  # 防止 log(0) 和 sqrt(0)

        if mask.sum() == 0:
            return torch.tensor(0.0, device=depth_est.device, requires_grad=True)

        masked_pred = depth_est[mask].clamp(min=epsilon)
        masked_gt = depth_gt[mask].clamp(min=epsilon)

        print("min depth_est:", masked_pred.min().item(), "min depth_gt:", masked_gt.min().item())

        d = torch.log(masked_pred) - torch.log(masked_gt)
        silog1 = (d ** 2).mean()
        silog2 = d.mean() ** 2

        var_term = silog1 - self.variance_focus * silog2
        var_term = torch.clamp(var_term, min=epsilon)  # 防止 sqrt(0) 或负数

        return torch.sqrt(var_term) * 10.0

    # def forward(self, depth_est, depth_gt, mask):
    #     epsilon = 1e-6
    #     if mask.sum() == 0:
    #         return torch.tensor(0.0, device=depth_est.device)
    #     print("min depth_est:", depth_est[mask].min().item(), "min depth_gt:", depth_gt[mask].min().item())
    #
    #     d = torch.log(depth_est[mask].clamp(min=epsilon)) - torch.log(depth_gt[mask].clamp(min=epsilon))
    #     var_term = (d ** 2).mean() - self.variance_focus * (d.mean() ** 2)
    #     var_term = torch.clamp(var_term, min=epsilon)
    #     return torch.sqrt(var_term) * 10.0
...
class bts(nn.Module):
    ...
    def forward(self, features):
        ...
        depth_8x8 = self.lpg8x8(plane_eq_8x8, self.bts_size)
        ...
        # 检查每个daspp输出是否有NaN
        if torch.isnan(daspp_3).any():
            print("NaN in daspp_3")
        if torch.isnan(daspp_6).any():
            print("NaN in daspp_6")
        if torch.isnan(daspp_12).any():
            print("NaN in daspp_12")
        if torch.isnan(daspp_18).any():
            print("NaN in daspp_18")
        if torch.isnan(daspp_24).any():
            print("NaN in daspp_24")

        if torch.isnan(iconv3).any():
            print("NaN in iconv3")
        ...
        depth_4x4 = self.lpg4x4(plane_eq_4x4, self.bts_size)
        ...
        depth_2x2 = self.lpg2x2(plane_eq_2x2, self.bts_size)
        ...
        if self.dataset == 'kitti':
            final_depth = final_depth * self.btd_size.view(-1, 1, 1, 1).float() / 715.0873
